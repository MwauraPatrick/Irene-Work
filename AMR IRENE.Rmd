...
IRENE_DATA_ANALYSIS_R %>% select(mo,Community) %>% melt(id.vars="Community") %>% filter(value!="Others") %>% ggplot(aes(x=Community,fill=value))+geom_bar(fill="steelblue")+ylab("E.coli")+stat_count(geom="text",aes(label=stat(count)), position = position_stack(vjust = .9),color="black")+ggtitle("Distribution of E.coli recovered")

IRENE_DATA_ANALYSIS_R %>% ggplot(aes(x=mo,fill=LPS))+geom_bar()+ylab("Number of Camels")+stat_count(geom="text",aes(label=stat(count)), position = position_stack(vjust = .9),color="black")+ggtitle("E.coli Recovered")
...
IRENE_DATA_ANALYSIS_R %>% select(Community,TEM,CTX_M) %>% melt(id.vars="Community") %>% filter(value!="NEG") %>% filter(value!="")%>% ggplot(aes(x=Community,fill=value))+geom_bar()+ylab("Gene Counts ")+stat_count(geom="text",aes(label=stat(count)), position = position_stack(vjust = .9),color="black")+ggtitle("Distribution of Genes")
